...
def screen():
    screen = ImageGrab.grab((764, 369, 964, 569)) # grab(x1, y1, x2, y2)
...
def wtf_pokemon():
    ...
    for x in range(1, 922):
        if x < 10:
            name_img = "img_or/00" + str(x) + ".png"
        elif x >= 10 and x < 100:
            name_img = "img_or/0" + str(x) + ".png"
        else:
            name_img = "img_or/" + str(x) + ".png"
        
        hash2 = CalcImageHash(name_img)
        result = CompareHash(hash1, hash2)

    # Педики - Ротом(id 810-814), Кьюрем, Деоксис
        if result < 2:
            print(pokemon_list[str(x) + ".png"] + " - " + str(result))
